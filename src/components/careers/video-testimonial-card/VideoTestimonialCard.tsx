...
export function VideoTestimonialCard({
  ...
  const { name, role, poster, youtubeId, videoSrc, captionsSrc } = testimonial;

  return (
    // Client asked to drop the name/role caption strip below the video —
    // just the video card now, so it's a single rounded element throughout.
    <div className="relative aspect-video bg-gray-100 rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      {youtubeId ? (
        <YouTubeFacade youtubeId={youtubeId} title={`${name}, ${role} at Lotus Care`} sizes={sizes} />
      ) : videoSrc ? (
