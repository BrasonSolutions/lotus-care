...
## Follow-up: continuous outer-petal sway

User liked the lotus and asked for a continuous, subtle idle animation on the outer, darker petals specifically (not the whole flower).

- `LotusMark.tsx`: tagged each facet whose color is the darkest tip shade (`#096972`) with `data-tier="outer"` (one per petal group — `bottom`/`left`/`right`/`upper-left`/`upper-right`; the pale `center` facet is untagged). Pure additive change, no visual effect on existing consumers (`CareersHero`'s static watermark).
- `globals.css`: switched the bloom entrance from `transform: scale()` to the standalone `scale` CSS property, freeing up `transform`/`rotate` for an independent, non-conflicting continuous animation. Added `@keyframes petal-sway` (±3deg rotate + subtle 1→1.04 scale breathing, 7s ease-in-out infinite) applied only to `[data-tier="outer"]`, scoped under `.lotus-bloom.in-view` so it starts only once the entrance bloom has triggered. Staggered via negative `animation-delay` per petal (0/-1.4/-2.8/-4.2/-5.6s) so the 5 outer tips sway out of phase, like independent petals in a light breeze, rather than in mechanical unison.
- Reduced motion: relies on the existing global `*` rule (zeroes `animation-duration`) plus an explicit `scale: none; rotate: none;` reset added to the reduced-motion block for `.lotus-bloom [data-petal]` (mirroring the pattern already used for `.pop-item`/`.reveal`).
- Found and fixed a selector bug during verification: the per-petal `animation-delay` rules initially combined `[data-petal="x"][data-tier="outer"]` as if both attributes were on the same element, but `data-tier` is on the `<path>` while `data-petal` is on its parent `<g>` — fixed with a descendant combinator (`[data-petal="x"] [data-tier="outer"]`), confirmed via computed-style checks that each petal now gets its correct staggered delay.

Verified: `tsc`/`lint`/`build` all pass. Confirmed via Playwright computed-style checks (animation-name/duration/delay per petal) and before/after screenshots ~2s apart showing visible independent movement of the outer petals only. Reduced-motion emulation confirmed `scale`/`rotate` both reset to `none` and animation duration collapsed.
